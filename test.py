...
fenetre.title("PREVISION DE VENTE")
fenetre.minsize(w,h)
fenetre.maxsize(w,h)
...
def executer():
	...
	try:
		...
		showwarning('Erreur', "Votre donnée n'est pas définie!")
...
def dataset():
	...
	interieur=Frame(canvas)
	canvas.create_window((0,0), window=interieur,anchor=NW,width=w-150,height=h-100)
	interieur.bind("<Configure>",myfunction)

	titre=Label(interieur, text="IMPORTATION DE DONNEE",font="times 14 bold", fg="blue", width=w-250)
	titre.pack(pady=15, padx=50, side=TOP)

	panel_data=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=50)
	panel_data.pack(side=TOP)

	panel_tableau=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=50)#, bg="#aaf000"
	panel_tableau.pack(side=TOP, padx=40)

	panel_visualisation=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=400) #, bg="#aaf033"
	panel_visualisation.pack(side=TOP, padx=40)

	panel_entete=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=500) #, bg="#aaf033"
	panel_entete.pack(side=TOP, padx=40)

	#contenu de panel data
	lab_importer = LabelFrame(panel_data, text="Choisir un fichier",width=(w-160)/4, padx=20, pady=20,relief=RIDGE)
	...
	lab_colonne = LabelFrame(panel_data, text="Nombre de colonne",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_colonne.pack(expand="yes",padx=20, side=LEFT)

	nb_col=StringVar()
	nb_col.set('{} colonnes'.format(col_lig[1]))
	Label(lab_colonne, textvariable=nb_col, width=20, fg="blue",font="times 15 bold").pack(side=LEFT)
	#Label(lab_colonne, text=" colonnes", width=7).pack(side=LEFT)

	lab_ligne = LabelFrame(panel_data, text="Nombre de ligne",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_ligne.pack(expand="yes",padx=0, side=LEFT)

	nb_lig=StringVar()
	nb_lig.set('{} lignes'.format(col_lig[0]))
	Label(lab_ligne, textvariable=nb_lig, width=20, fg="blue",font="times 15 bold").pack(side=LEFT)
	#Label(lab_ligne, text=" lignes",width=5).pack(side=LEFT)


	Label(panel_tableau, text="CONFIGURATION",font="times 14 bold", fg="blue", width=w-160).pack(padx=10, pady=10)


	#contenu de panel data_configuration de base
	lab_dataset = LabelFrame(panel_visualisation, text="Type de donnée",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	...
	lab_entrainement = LabelFrame(panel_visualisation, text="Type d'entrainement",width=(w-160)/4, padx=20, pady=20,relief=RIDGE)
	...
	lab_index = LabelFrame(panel_visualisation, text="Variable d'indexation",width=(w-160)/4, padx=20, pady=20,relief=RIDGE)
	...
	lab_target = LabelFrame(panel_visualisation, text="Variable cible (target)",width=(w-160)/4, padx=20, pady=20,relief=RIDGE)
	...
	Label(panel_entete, text="APERCU DE LA DONNEE",font="times 14 bold", fg="blue", width=w-160).pack(padx=10, pady=10)
...
def general():
	...
	def affichage_model():
		...
		im_arbre.bar(courses, values,color='b', width = 0.4, linewidth = 5,ecolor = 'red', capsize = 30) 
	...
	train_sizes, train_score, val_score=learning_curve(Tree, X_train,y_train, cv=4, scoring=type_score,train_sizes=np.linspace(0.1, 1, 4))
	...
	def courbe_d_apprentissage():
		...
		im_arbre.legend()
		im_arbre.set_title("Courbe d'apprentissage de l'arbre de décision")
	...
	titre=Label(interieur, text="REPRESENTATION DES MODELES", fg="blue",font="times 15 bold")
	titre.pack(pady=20, side=TOP)

	panel_train_score=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=50)
	panel_train_score.pack(side=TOP)

	panel_arbre=PanedWindow(interieur, orient=HORIZONTAL, width=w-200,height=200) #, bg="#aaf033"
	panel_arbre.pack(side=TOP, padx=40, pady=20)

	affichage_model()

	panel_titre=PanedWindow(interieur, orient=HORIZONTAL, width=w-200,height=200) #, bg="#aaf033"
	panel_titre.pack(side=TOP, padx=40, pady=10)

	panel_learning_curve=PanedWindow(interieur, orient=HORIZONTAL, width=w-200,height=400)#, bg="#aaf000"
	panel_learning_curve.pack(side=TOP, padx=20)

	courbe_d_apprentissage()


	lab_score = LabelFrame(panel_train_score, text="Score moyen estimé",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_score.pack(expand="yes",padx=20, side=LEFT)
	
	score_estime=StringVar()
	score_estime.set(pred)
	#score_estime.set(pred_tree)
	Label(lab_score, textvariable=score_estime, width=20, fg="blue",font="times 15 bold").pack(side=LEFT)
	#Label(lab_score, text=" colonnes", width=n7).pack(side=LEFT)

	lab_nomtarget = LabelFrame(panel_train_score, text="Variable selectionnée à predire",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_nomtarget.pack(expand="yes",padx=20, side=LEFT)
	
	nom_target=StringVar()
	nom_target.set(type_target)
	Label(lab_nomtarget, textvariable=nom_target, width=20, fg="blue",font="times 15 bold").pack(side=LEFT)

	lab_model = LabelFrame(panel_train_score, text="Nombre de modeles entrainés",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	...
	Label(lab_model, textvariable=nb_model_est, width=20, fg="blue",font="times 15 bold").pack(side=LEFT)
	...
	Label(panel_titre, textvariable=titre_tab,font="times 14 bold", fg="blue", width=w-160).pack(padx=10, pady=20)
...
def model():
	...
	tab_ridge=["Ridge","SKLEARN","Lineaire",pred_ridge,score_ridge,t_ridge,"Actif"]
	tab_sgd=["SGD","SKLEARN","Lineaire",pred_sgd,score_sgd,t_sgd,"Actif"]
	...
	tab_rna=["RNA","SKLEARN","Reseau de Neurone",pred_rna,score_rna,t_rna,"Actif"]
	tab_rforest=["RForest","SKLEARN","Foret Aleatoire",pred_rforest,score_rforest,t_rforest,"Actif"]
	...
	if type_dataset==liste_dataset[0]:
		if type_entrainement==liste_entrainement[0]:
			pred_bayes='Indefinie'
			statut_bayes='Inactif'
			score_bayes='Indefinie'
			t_bayes='Indefinie'
		else:
			statut_bayes='Actif'
	if type_dataset==liste_dataset[1]:
...
def prevision():
	...
	interieur=Frame(canvas)
	canvas.create_window((0,0), window=interieur,anchor=NW,width=w-150,height=h)
	interieur.bind("<Configure>",myfunction)


	titre=Label(interieur, text="PAGE DE LA PREVISION", fg="blue",font="times 15 bold")
	titre.pack(pady=25, side=TOP)

	panel_selection_date=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=50)
	panel_selection_date.pack(side=TOP)

	panel_graphe=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=400) #, bg="#aaf033"
	panel_graphe.pack(side=TOP, padx=40, pady=20)

	panel_description=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=200)#, bg="#aaf000"
	panel_description.pack(side=TOP, padx=40)


	lab_select_model = LabelFrame(panel_selection_date, text="Paramètres de prévision",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	...
	def select_model_pref(event):
		...
		if event:
			...
			print("Modele preferer : ", mod_pref)
	...
	def plot_prevision():
		...
		if type_dataset==liste_dataset[0]:
			_X=data.drop(type_target,axis=1)
			_y=data[type_target]

			taille_liste_col=len(liste_colonne)

			from sklearn.linear_model import LinearRegression, Ridge
			rid = Ridge()

			def eval_regression(model,X_scatter,y):
				model.fit(X_scatter,y)
				res_pred=model.predict(X_scatter)
				plt.scatter(X_scatter,y, s=100,alpha=0.5)
				plt.plot(X_scatter,res_pred, color='red', label="Droite de régression y=aX+b")
				plt.title("Prévision de regression Lineaire") 
				plt.xlabel("x: {}".format(var_x)) 
				plt.ylabel("y: {}".format(type_target)) 
				#plt.xticks(x)
				plt.legend()
				plt.show() 
			

			for i in range(taille_liste_col):
				if var_x==liste_colonne[i]:
					_X_scatter=_X.iloc[:,i:i+1]
			
			if type_entrainement==liste_entrainement[0]:    
				eval_regression(rid,_X_scatter,_y)    
		...
		if type_dataset==liste_dataset[1]:
			...
			fig = res_arima.plot_predict(1,jour_pred)
			#plt.xlabel('Date')

			#plt.xlabel("n-ieme de la date") 
			plt.xlabel("{} indexé(e)".format(type_index)) 
			plt.ylabel("Valeur de prédiction") 
			plt.title("Forecasting selon le modele ARIMA") 
			#plt.xticks(type_index)
			plt.show()
	...
	lab_model = LabelFrame(panel_selection_date, text="Commande de prévision",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_model.pack(expand="yes",padx=10, side=LEFT)

	Button(lab_model, text="Faire prédire", width=15, fg="white", bg='green', command=faire_predire).pack(side=LEFT, padx=10)
	#Label(lab_model, text=" lignes",width=5).pack(side=LEFT)

	forecast_plot=StringVar()
	if type_dataset==liste_dataset[0]:
		forecast_plot.set('Afficher scatter')
		
	if type_dataset==liste_dataset[1]:
		forecast_plot.set('Afficher forecasting')

	cmd=Button(lab_model, textvariable=forecast_plot, width=15, fg="white", bg='green', command=plot_prevision)
	#Label(lab_model, text=" lignes",width=5).pack(side=LEFT)
	cmd.pack(side=LEFT, padx=10)

	lab_model = LabelFrame(panel_selection_date, text="Résultat de prédiction ",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_model.pack(expand="yes",padx=10, side=LEFT)

	resultat=StringVar()
	resultat.set('None')

	Label(lab_model, textvariable=resultat,width=15, font='sans 14 bold', fg="blue").pack()
	# GRAPHE DE PREVISION
	...
	y_prevision=y_reel.copy()
	y_prevision.append(pred_moyenne)

	f_prev = Figure(figsize=(10,5), dpi=100)

	pp = f_prev.add_subplot(111)

	pp.plot(y_prevision, label='Prévision',c='red')#, c='red'
	pp.plot(SMA, label='Moyenne Mobile' ,lw=3,ls=':', c='green')#, c='green'
...
def features():
	...
	interieur=Frame(canvas)
	canvas.create_window((0,0), window=interieur,anchor=NW,width=w-150,height=h+400)
	interieur.bind("<Configure>",myfunction)


	titre=Label(interieur, text="CARACTERISTIQUES DES VARIABLES", fg="blue",font="times 15 bold")
	titre.pack(pady=15, side=TOP)

	panel_features1=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=50)
	panel_features1.pack(side=TOP)

	panel_features2=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=800) #, bg="#aaf033"
	panel_features2.pack(side=TOP, padx=40, pady=20)

	panel_features3=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=400)#, bg="#aaf000"
	panel_features3.pack(side=TOP, padx=40)


	Label(panel_features1,text="Valeur manquante : ").pack(side=LEFT, padx=20)

	val_manq=StringVar()
	val_manq.set('0.00 %')
	Label(panel_features1, textvariable=val_manq, width=20, fg="blue",font="times 12 bold").pack(side=LEFT)

	Label(panel_features1,text="Variable cible (à prédire) : ").pack(side=LEFT, padx=20)

	nb_lig=StringVar()
	nb_lig.set(type_target)
	Label(panel_features1, textvariable=nb_lig, width=20, fg="blue",font="times 12 bold").pack(side=LEFT)
	...
	# VAriatin de variable predictive
	f_var = Figure(figsize=(10,5), dpi=100)

	p_var = f_var.add_subplot(111)

	p_var.plot(list(data[type_target]), label=' {}'.format(type_target), c='red')
	#p_var.set_yticklabels('Valeur')

	#if type_dataset==liste_dataset[1]:
		#p_var.set_xticks(data[type_index])
	
	p_var.legend()
	p_var.set_title('Variation de {}'.format(type_target))

	grp_var = FigureCanvasTkAgg(f_var, master=panel_features3)
	canv3=grp_var.get_tk_widget().grid()
...
def score():
	...
	titre=Label(interieur, text="COMPARAISON DES SCORES D'ENTRAINEMENT ET DE VALIDATION", fg="blue",font="times 15 bold")
	titre.pack(pady=20, side=TOP)

	panel_score1=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=600)
	panel_score1.pack(side=TOP)

	panel_score2=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=800) #, bg="#aaf033"
	...
	dict_metric_order = OrderedDict(sorted(dict_metric.items(), key=lambda x: x[1],reverse=False))


	dict_metric_keys = list(dict_metric_order.keys()) 
	dict_metric_keys_values = list(dict_metric_order.values())

	# fonction qui affiche le model

	def affichage_metric():
		fig_arbre=Figure(figsize=(5,5),dpi=100)

		im_arbre = fig_arbre.add_subplot(111)
		im_arbre.barh(dict_metric_keys, dict_metric_keys_values,alpha=0.5,color='yellow', height = 0.3,linestyle = 'solid', hatch ='/') # marron, color ='b', width = 0.5
		im_arbre.set_title("Metriques")
		im_arbre.set_xlabel('Valeur')

		graph_arbre = FigureCanvasTkAgg(fig_arbre, master=frm_score)
		canvas_arbre = graph_arbre.get_tk_widget()
		canvas_arbre.pack(padx=0)

	affichage_metric()

	def erreur_histogramme():

		fig1=Figure(figsize=(5,5),dpi=100)
		im1 = fig1.add_subplot(111)

		im1.hist(erreur_hist,bins=30)

		im1.set_title("Histogramme d'erreur")
		#im1.set_xlabel("Prédiction")
		#im1.set_ylabel("Valeur")
...
def historique():
	...
	titre=Label(interieur, text="TABLEAU RECAPUTILATIF DES HISTORIQUES", fg="blue",font="times 15 bold")
	titre.pack(pady=50, side=TOP)

	panel_score1=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=50)
	panel_score1.pack(side=TOP)

	panel_score2=PanedWindow(interieur, orient=HORIZONTAL, width=w-160,height=900) #, bg="#aaf033"
	panel_score2.pack(side=TOP, padx=40, pady=20)

	lab_titre1 = LabelFrame(panel_score1, text="Affichage",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_titre1.pack(expand="yes",padx=40, side=LEFT)

	Button(lab_titre1, text='Afficher', width=20, bg='green', fg='white', command=View).pack()

	lab_titre = LabelFrame(panel_score1, text="Mise à jour",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_titre.pack(expand="yes",padx=40, side=LEFT)
	Button(lab_titre,text='Mettre à jour', width=20, bg='green', fg='white').pack()

	lab_titre2 = LabelFrame(panel_score1, text="Triage",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_titre2.pack(expand="yes",padx=40, side=LEFT)
	Button(lab_titre2,text='Trier', width=20, bg='green', fg='white').pack()


	lab_titre3 = LabelFrame(panel_score1, text="Impression",width=(w-160)/4, padx=20, pady=20, relief=RIDGE)
	lab_titre3.pack(expand="yes",padx=40, side=LEFT)
	Button(lab_titre3,text='Imprimer', width=20, bg='green', fg='white').pack()
...
#panel en contenu
pane_contenu=PanedWindow(pane_sidebar_contenu, orient=VERTICAL, width=w-160, height=h,relief=FLAT)
...
def Heure():
	Label_Heure.config(text=time.strftime('%a %d %b %Y   %H:%M:%S'))
	Label_Heure.after(200, Heure) 

panel_tete.add(Label(panel_tete,text="PREVISION DE VENTE",bg='#4e73df',fg="white",font="times 15 bold italic"))
Label_Heure=Label(width=22,bg='#4e73df', fg='white',font='times 13 bold italic')
Label_Heure.pack()
panel_tete.add(Label_Heure)

Heure()
btn_predire=Button(text="/>  Prédire ", bg='green', fg='orange',font="times 13 bold italic ",width=10,relief=GROOVE, command=executer)
btn_predire.pack()
...
def apropos():
	...
	photo_accueil = PhotoImage(file="icon/accueil4.png")
...
recherche=Entry(textvariable=value,width=25,font='sans 10 italic')
recherche.pack()
...
panel_tete.add(Button(panel_tete,text="ACCUEIL",width=15,font='times 11 bold italic',fg='orange',bg='#4e73df',relief=FLAT, command=accueil))
panel_tete.add(Button(panel_tete,text="PARAMETRE",width=15,font='times 11 bold italic',fg='orange',bg='#4e73df',relief=FLAT, command=parametre))
panel_tete.add(Button(panel_tete,text="A PROPOS",width=15,font='times 11 bold italic',fg='orange',bg='#4e73df',relief=FLAT, command=apropos))
panel_tete.add(Label(panel_tete,text="",width=15,bg='#4e73df',relief=FLAT))

societe=StringVar()
societe.set("PROFILE")
mb=  Menubutton ( panel_tete, textvariable=societe,width=10,font='times 11 bold italic',fg='orange', relief=FLAT ,bg='#4e73df')
...
def profile():   
	...
	requete_profile="SELECT * from login"
	query=c.execute(requete_profile)
	for res in query:
...
mb.menu.add_command ( label="Mon profile", command=profile)
mb.menu.add_separator()
mb.menu.add_command ( label="PDV v1.0.0",command=version)
mb.menu.add_separator()
mb.menu.add_command ( label="Déconnecter",command=deconnecter)
...
panel_sidebar.add(Button(panel_sidebar,text="Dataset",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=login))
panel_sidebar.add(Button(panel_sidebar,text="Générale",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=general))
panel_sidebar.add(Button(panel_sidebar,text="Modèles",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=model))
panel_sidebar.add(Button(panel_sidebar,text="Features",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=features))
panel_sidebar.add(Button(panel_sidebar,text="Scores",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=score))   
panel_sidebar.add(Button(panel_sidebar,text="Prévision",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=prevision))
panel_sidebar.add(Button(panel_sidebar,text="Historiques",height=2,width=15,font='Copperplate 10',fg='white',bg='#4e73df',relief=GROOVE, command=historique))
panel_sidebar.add(Label(panel_sidebar,text="",pady=h,height=10, width=15,font='Copperplate 10',fg='white',bg='#4e73df'))

################################### CONTENU   ###################################

frame_accueil=Frame(pane_contenu, width=w-160, height=h)
frame_accueil.pack(padx=20, side=LEFT)
frame_accueil.place(x=2,y=10)
